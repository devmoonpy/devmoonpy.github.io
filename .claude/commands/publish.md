...
`knowledge-site/content/` 등의 변경사항을 검증하고, 통과하면 사용자 확인을 받아 commit/push한다. push되면 GitHub Actions가 자동으로 `https://devmoonpy.github.io/`를 갱신한다.
...
7. push 후, GitHub Actions 워크플로(`Deploy Pages`)가 실행 중이라는 것과 몇 분 뒤 `https://devmoonpy.github.io/`에서 확인할 수 있다는 것을 안내한다. `gh run watch --repo devmoonpy/devmoonpy.github.io`로 진행 상황을 지켜볼 수 있다(이 저장소엔 `upstream`이라는 리모트도 있어서 `--repo`를 꼭 명시해야 함 — 안 그러면 `jackyzha0/quartz`로 잘못 잡힐 수 있음).
